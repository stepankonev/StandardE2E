...
class HDMapBEVAdapter(AbstractAdapter):
    ...
    One channel per element type listed in ``include_types`` (default: every
    member of :class:`MapElementType` in declaration order). Polygons are
    drawn filled; polylines are drawn with ``polyline_thickness``; points
    ...
    ``(C, H, W)`` with ``C = len(include_types)``,
    ...
    Args:
        ...
        include_types: ordered list of ``MapElementType`` determining channel
            order. Defaults to all enum members in declaration order.
    ...
    def __init__(
        ...
        include_types: list[MapElementType] | None = None,
        ...
        self._include_types = (
            list(include_types) if include_types is not None else list(MapElementType)
        )
        self._type_to_channel = {t: i for i, t in enumerate(self._include_types)}
    ...
    def output_shape(self) -> tuple[int, int, int]:
        ...
        return (len(self._include_types), h, w)
